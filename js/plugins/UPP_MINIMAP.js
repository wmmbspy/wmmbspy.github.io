...
* ||  upp_m_hideEventDrawing 1
*
* upp_m_hideThisEventDrawing - Hides this event from the minimap by setting
*							   it's color to rgba(0, 0, 0, 0)
*
* 
...
* 
* upp_m_changeThisEventColor - Changes this event's color
* || Example:
* ||  upp_m_changeThisEventColor 0 rgba(150, 255, 255, 0.5) (leave the 0 there)
*
* REGION COLORS:
...
*
* script calls:
* this.onDeathDisappear(); hides this event drawing.
*/
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
	...
	if(command == 'upp_m_hideThisEventDrawing')
	{
		for(i=0;i<_shownEvents;i++)
		{
			if(_minimapEvents[i].id==this._eventId)
			{
				_minimapEvents[i].color = "rgba(0, 0, 0, 0)";
			}
		}
	...
	if(command == 'upp_m_changeEventColor')
	{
		...
		if(command == 'upp_m_changeThisEventColor')
	{
		var string="";
		for(i=1;i<args.length;i++)
		{
...
Game_CharacterBase.prototype.thisId = function() {
	
};

Game_Interpreter.prototype.thisId = function(ID) {
		for(i=0;i<_shownEvents;i++)
		{
			if(_minimapEvents[i].id==ID)
			{
				_minimapEvents[i].color = "rgba(0, 0, 0, 0)";
			}
		}
  ...
  Game_Event.prototype.onDeathDisappear = function() {
		for(i=0;i<_shownEvents;i++)
		{
			if(_minimapEvents[i].id==this._eventId)
			{
  ...
  Game_Interpreter.prototype.onDeathDisappear = function() {
		for(i=0;i<_shownEvents;i++)
		{
			if(_minimapEvents[i].id==this._eventId)
			{
...
Game_Event.prototype.thisId = function() {
				var Eid = this.battler()._enemyId;
				Game_Interpreter.hideEventDrawing(Eid);
}
...
var makeItInvisible = function(string)
{
				_minimapEvents[this._eventId].color = string;
}